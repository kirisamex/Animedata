...
using CVListRow = Client.ClientDataSet.ClientDataSet.CVListRow;
using CVHistRow = Client.ClientDataSet.ClientDataSet.CVHistRow;
    ...
    public partial class CVManage : Form
    {
        ...
        private void LoadCVDataSet(DataSet ds)
        {
            try
            {
                ...
                {
                    CVListRow cvRow = cvList.NewCVListRow();
                ...
                foreach (CVListRow dr in cvList)
                {
                    cvids.Add(dr.CVID);
                }
                ...
                if (cvids.Count != 0)
                {
                    ...
                    {
                        CVHistRow cvRow = cvHist.NewCVHistRow();
        ...
        private void ShowCVInfo(DataSet ds)
        {
            ...
            try
            {
                foreach (CVListRow dr in cvList.Rows)
                {
        ...
        private bool ChangeCVInfo()
        {
            ...
            foreach (DataGridViewRow dr in cvdataGridView.Rows)
            {
                ...
                foreach (CVListRow cvr in targetCV)
                {
        ...
        private void ShowCVHistInfo(CV cvInfo)
        {
            try
            {
                ...
                foreach (CVHistRow dr in targetList)
                {
        ...
        private void CVHistdataGridView_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                Main.Mainfm.SimpleSearch(CVHistdataGridView.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString());
                Main.Mainfm.Focus();
            }
        }
