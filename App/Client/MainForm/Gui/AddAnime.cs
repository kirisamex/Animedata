...
namespace Client.MainForm.Gui
{
    public partial class AddAnime : Form
    {
        ...
        /// </summary>
        /// <param name="control">操作种类</param>
        public AddAnime(AnimeCommand.Command control)
        {
            InitializeComponent();
            cmd = control;
        }
        ...
        /// 文本框信息设定
        /// </summary>
        public void LoadFormWithAdd(AnimeCommand.Command ctr)
        {
            switch (ctr)
            {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (CommandAnimeInfo(cmd) == true)
            {
                Main.Mainfm.DataGridViewReload();
        ...
        /// <returns></returns>
        private bool CommandAnimeInfo(AnimeCommand.Command ctr)
        {
            //填写完整性检查以及格式检查
            ...
            try
            {
                ...
                if (ctr == AnimeCommand.Command.Add)
                {
                    if (anime.Insert())
                    {
                        return true;
                ...
                else if (ctr == AnimeCommand.Command.Update)
                {
                    if (anime.Update())
                    {
                        return true;
        ...
        private List<PlayInfo> PlayInfoSeries(string animeNo)
        {
            ...
            //信息作成
            int NextPlayinfoID = service.GetMaxInt(3, animeNo) + 1;
            ...
            for (int i = 0; i < PlayInfoDataGridView.RowCount; i++)
            {
                ...
                if ((cmd == AnimeCommand.Command.Add)
                    ...
                    && cmd == AnimeCommand.Command.Update)
                       )
                {
                    pInfo.ID = NextPlayinfoID + NewplayinfoCount;
                    NewplayinfoCount++;
                }
                else if (PlayInfoDataGridView.Rows[i].Cells[PLAYINFOIDCLN].Value != null && cmd == AnimeCommand.Command.Update)
                {
        ...
        private List<Character> CharacterInfoSeries(string animeNo)
        {
            ...
            for (int i = 0; i < CharacterInfoDataGridView.RowCount; i++)
            {
                ...
                //设定编号
                if (CharacterInfoDataGridView.Rows[i].Cells[CHARACTERNOCLN].Value != null && cmd == AnimeCommand.Command.Update)
                {
        ...
        private void PlayInfoDataGridView_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            if (CurrnetCell != null && CurrnetCell.OwningColumn.Name == "company")
            {
                ...
                try
                {
                    DataSet ds = service.LoadCompanyName();
                    dt = ds.Tables[0];
